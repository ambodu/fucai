...
export default function DataTicker() {
  ...
  return (
    <section className="max-w-[1200px] mx-auto px-4 lg:px-6 py-6 space-y-4">
      {/* Latest draw */}
      {latest && (
        <div className="bg-white shadow-apple rounded-2xl p-4 lg:p-5">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div className="w-px h-10 bg-[#ebebed]" />
      ...
      <div className="grid grid-cols-4 gap-3">
        {NAV_ITEMS.map(item => (
          <Link
            key={item.label}
            href={item.href}
            className="flex flex-col items-center gap-1.5 p-3 rounded-2xl bg-white shadow-apple-sm hover:shadow-apple transition-all"
          >
            <div className="w-10 h-10 rounded-xl flex items-center justify-center bg-[#0071e3]/8">
              <item.icon size={20} className="text-[#0071e3]" />
            </div>
            <div className="text-xs font-semibold text-[#1d1d1f]">{item.label}</div>
            <div className="text-[10px] text-[#6e6e73] hidden sm:block">{item.desc}</div>
          </Link>
        ))}
      </div>
    </section>
