    ...
    <div className="space-y-6">
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <p className="text-xs text-[#6e6e73] mt-0.5">基于近50期数据统计分析，仅供参考</p>
        </div>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        {prediction.positions.map(pos => (
          <div key={pos.position} className="bg-white rounded-2xl shadow-apple p-5">
            <h3 className="text-sm font-semibold mb-4 text-[#0071e3]">{pos.positionLabel}分析</h3>

            {/* Hot digits */}
            <div className="mb-4">
              <div className="text-[11px] text-[#6e6e73] font-medium mb-2">热号（高频出现）</div>
            ...
            {/* Cold digits */}
            <div className="mb-4">
              <div className="text-[11px] text-[#6e6e73] font-medium mb-2">冷号（长期未出）</div>
              <div className="space-y-1.5">
            ...
            {/* Recommended */}
            <div className="pt-3 border-t border-[#ebebed]">
      ...
      <div className="bg-white rounded-2xl shadow-apple p-5">
        <h3 className="text-sm font-semibold mb-4 text-[#1d1d1f]">综合指标分析</h3>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-[#f5f5f7] rounded-xl p-3.5">
            <div className="text-[11px] text-[#6e6e73] mb-1">和值范围</div>
            <div className="text-base font-bold text-[#1d1d1f]">{prediction.sumRange.min} - {prediction.sumRange.max}</div>
            <div className="text-xs text-[#6e6e73]">均值: {prediction.sumRange.avg}</div>
          </div>
          <div className="bg-[#f5f5f7] rounded-xl p-3.5">
            <div className="text-[11px] text-[#6e6e73] mb-1">跨度范围</div>
            <div className="text-base font-bold text-[#1d1d1f]">{prediction.spanRange.min} - {prediction.spanRange.max}</div>
            <div className="text-xs text-[#6e6e73]">均值: {prediction.spanRange.avg}</div>
          </div>
          <div className="bg-[#f5f5f7] rounded-xl p-3.5">
            <div className="text-[11px] text-[#6e6e73] mb-1">常见大小形态</div>
            <div className="flex gap-1 flex-wrap mt-1">
              {prediction.likelyPatterns.bigSmall.map(p => (
                <span key={p} className="px-2 py-0.5 bg-[#0071e3]/8 text-[#0071e3] rounded-lg text-[11px] font-medium">{p}</span>
              ))}
            </div>
          </div>
          <div className="bg-[#f5f5f7] rounded-xl p-3.5">
            <div className="text-[11px] text-[#6e6e73] mb-1">常见奇偶形态</div>
            <div className="flex gap-1 flex-wrap mt-1">
              {prediction.likelyPatterns.oddEven.map(p => (
                <span key={p} className="px-2 py-0.5 bg-[#8b5cf6]/8 text-[#8b5cf6] rounded-lg text-[11px] font-medium">{p}</span>
              ))}
            </div>
          </div>
        </div>
